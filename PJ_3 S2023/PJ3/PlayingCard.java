...
class OneDeck {
    ...
    public List<Card> deal(int numberCards) throws PlayingCardException {
        // implement this method!
        int originalSize = remainSize();
        if(numberCards > originalSize){
            throw new PlayingCardException("No enough cards");
        }
        List<Card>dealtCards = playDeck.subList(originalSize-numberCards,originalSize);
        List<Card>newlist = new ArrayList<>(dealtCards);
        playDeck.removeAll(dealtCards);
        return newlist;
    }
